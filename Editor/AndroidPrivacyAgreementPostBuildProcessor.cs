...
class AndroidPrivacyAgreementPostBuildProcessor : IPostGenerateGradleAndroidProject
{
    ...
    public void OnPostGenerateGradleAndroidProject(string path)
    {
        ...
        //@Lezhi 202050604 需要先刪除重复的
        if (content.Contains(privacyImport))
        {
            content = content.Replace(privacyImport, "");
        }
        var privacyContent = privacyString.Replace("{APP_NAME}", privacyAppName).Replace("{APP_EMAIL}", privacyEmail) + "\n";
...
//                  ""应监管要求,请你务必审慎阅读、充分理解“隐私政策”各条款，包括但不限于：为了向你提供精准广告、优化游戏体验等，我们需要收集你的设备信息等个人信息。\n"" +
                    ...
                    ""1.5 软件所收集信息说明：\n"" +
                    ""\n"" +
                    "" 数据类型：地理定位，位置 Location，粗略位置 Coarse Location\n"" +
					"" 使用者：字节跳动SDK， Gromore SDK，友盟SDK，快手SDK，优量汇SDK\n"" +
                    "" 使用目的：第三方广告：我们将收集粗略位置信息，用于广告投放与监测归因 分析服务：我们将收集粗略位置信息，用于数据统计分析。\n"" +
                    "" 是否与身份关联： 收集数据的主要目的是用于“第三方广告：显示广告” “分析服务：游戏运营”，在获得用户授权的情况下，才获取粗略位置信息。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告：显示广告” “分析服务：游戏运营”。\n"" +
                    ""\n"" +

                    "" 数据类型：存储 SD卡数据 SDCard Data\n"" +
					"" 使用者：友盟SDK\n"" +
                    "" 使用目的：App功能：我们将读写运行过程中的崩溃信息、性能数据，以方便应用进行优化。\n"" +
                    "" 是否与身份关联：读写数据的主要目的是用于“App功能：获取程序崩溃及性能数据，方便应用优化”，在获得用户授权的情况下，才存储相关数据。\n"" +
                    "" 用于何种追踪目的：读写数据的主要目的是用于“App功能：获取程序崩溃及性能数据，方便应用优化”。\n"" +
                    ""\n"" +

                    "" 数据类型：设备传感器，传感器列表，陀螺仪传感器\n"" +
					"" 使用者：友盟SDK，优量汇SDK\n"" +
                    "" 使用目的：App功能：我们将监听陀螺仪传感器，以分析用户的使用习惯。\n"" +
                    "" 是否与身份关联：监听陀螺仪传感器的主要目的是用于“分析用户使用习惯，方便应用优化”，在获得用户授权的情况下，才获取传感器信息。\n"" +
                    "" 用于何种追踪目的：读写数据的主要目的是用于“App功能：分析用户使用习惯，方便应用优化”。\n"" +                   
                    ""\n"" +

                    "" 数据类型：使用数据 Usage Data， 产品交互 Product Interaction， 广告数据 Advertising Data\n"" +
					"" 使用者：字节跳动SDK，Gromore SDK，友盟SDK\n"" +
                    "" 使用目的：第三方广告：我们将统计展示 、点击 、转化广告数据，以用于广告主统计投放结果，向您提供帮助调整广告变现策略的服务 分析服务：我们将统计相关信息，用于数据统计分析。\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“第三方广告：显示广告” “分析服务：游戏运营”，在获得用户授权的情况下，才获取相关数据。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告：显示广告” “分析服务：游戏运营”。\n"" +
                    ""\n"" +
                    ...
                    "" 是否与身份关联：收集数据的主要目的是用于“App功能：减少崩溃、确保服务器正常运行、提升可扩展性和性能”，在获得用户授权的情况下，才获取崩溃与性能数据。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“App功能：减少崩溃、确保服务器正常运行、提升可扩展性和性能”。\n"" +
                    ""\n"" +

					"" 数据类型：应用安装列表\n"" +
					"" 使用者：字节跳动SDK，Gromore SDK，友盟SDK，快手SDK，优量汇SDK\n"" +
                    "" 使用目的：为帮助我们更好地了解相关服务的运行情况，以便确保运行与提供服务的安全，我们可能记录网络日志信息，以及使用软件及相关服务的频率，总体安装、使用情况、性能数据等信息。\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“第三方广告-显示广告” “分析服务：游戏运营”，在获得用户授权的情况下，才获取软件安装列表。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告-显示广告” “分析服务：游戏运营”。\n"" +
                    ""\n"" +

                    "" 数据类型：其他数据 Other Data， 其他数据类型 Other Data Types 硬件序列号\n"" +
					"" 使用者：字节跳动SDK，Gromore SDK，友盟SDK，快手SDK，优量汇SDK\n"" +
                    "" 使用目的：第三方广告：技术上我们将收集一些设备信息（例如，硬件序列号、系统语言、屏幕高宽、屏幕方向、屏幕DPI信息） 分析服务：我们将收集一些设备信息（例如，硬件序列号、系统语言、屏幕高宽、屏幕方向、屏幕DPI信息），用于数据统计分析。\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“第三方广告-显示广告” “分析服务：游戏运营”，在获得用户授权的情况下，才获取相关数据。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告-显示广告” “分析服务：游戏运营”。\n"" +
                    ""\n"" +

					"" 数据类型：OAID，SSID，BSSID，IMSI，MAC地址，IMEI，Identifiers，IP，User ID，Device ID，Android ID，硬件设备序列号\n"" +
					"" 使用者：游戏自身，字节跳动SDK， Gromore SDK，友盟SDK，快手SDK，优量汇SDK\n"" +
                    "" 使用目的： 辅助开发/广告/推送/框架。\n"" +
                    "" 详细政策说明："" +
                    "" （1）字节跳动SDK隐私政策（https://developer.open-douyin.com/docs/resource/zh-CN/dop/operation-standard/service-protocol/douyin-sdk-privacy-policy）；\n"" +
                    "" （2）穿山甲Gromore聚合广告SDK隐私政策（https://www.pangle.cn/privacy）；\n"" +
                    "" （3）友盟SDK隐私政策（https://www.umeng.com/policy）；\n"" +
                    "" （4）快手SDK隐私政策（https://u.kuaishou.com/home/detail/1290）；\n"" +
                    "" （5）优量汇SDK隐私政策（https://cdcslm.sinaapp.com/ylhsdk_yssm_cslm.html）；\n"" +
                    ""\n"" +

                    ""1.6 自启动和关联启动说明：\n"" +
                    ""\n"" +
                    "" 为了让您体验APP消息离线提醒功能，查看第三方应用，更全面体验APP功能，提供更优质的服务，在您同意本隐私政策后，APP在必要时，会开启自启动或关联启动功能\n"" +
                    ""\n"" +
